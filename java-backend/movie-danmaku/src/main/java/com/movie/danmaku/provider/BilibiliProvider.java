package com.movie.danmaku.provider;

import com.movie.danmaku.dto.DanmakuDto;
import com.movie.danmaku.parser.DanmakuXmlParser;
import bilibili.dm.BilibiliDm;
import com.movie.danmaku.throttle.RateLimiterService;
...
public class BilibiliProvider implements DanmakuProvider {
    ...
    public BilibiliProvider(OkHttpClient client, RateLimiterService rateLimiter, DanmakuXmlParser xmlParser) {
        this.client = client;
    ...
    public List<DanmakuDto> fetchDanmaku(String cid) throws Exception {
        // Try protobuf first, fallback to XML
        try {
            return fetchDanmakuProtobuf(cid);
        } catch (Exception e) {
            log.warn("Protobuf fetch failed for cid {}, falling back to XML: {}", cid, e.getMessage());
    ...
    private String fetchCidByBvid(String bvid) throws Exception {
        ...
        // Simple JSON parsing for [{"cid":xxx,...}]
    ...
    private List<DanmakuDto> fetchDanmakuProtobuf(String cid) throws Exception {
        List<DanmakuDto> allDanmakus = new ArrayList<>();

        for (int segmentIndex = 1; segmentIndex <= 100; segmentIndex++) {
            String url = BILIBILI_API_URL + "/x/v2/dm/web/seg.so?oid=" + cid + "&type=1&segment_index=" + segmentIndex;
            rateLimiter.acquire();
            ...
            try (Response response = client.newCall(request).execute()) {
                if (response.code() == 304 || response.body() == null) {
                    break;
                }

                if (!response.isSuccessful()) {
                    throw new RuntimeException("Failed to fetch protobuf: HTTP " + response.code());
                }
                ...
                BilibiliDm.DmSegMobileReply reply = BilibiliDm.DmSegMobileReply.parseFrom(data);
                if (reply.getElemsCount() == 0) {
                    break;
                }

                for (BilibiliDm.DanmakuElem elem : reply.getElemsList()) {
                    DanmakuDto danmaku = convertProtobufDanmaku(elem);
                    if (danmaku != null) {
                        allDanmakus.add(danmaku);
                    }
                }
            }
        }
    ...
    private DanmakuDto convertProtobufDanmaku(BilibiliDm.DanmakuElem elem) {
        ...
        double time = elem.getProgress() / 1000.0;
        String color = String.format("#%06x", elem.getColor() & 0xFFFFFF);

        return new DanmakuDto(time, elem.getContent(), type, color, "bilibili");
    }
    ...
    private String extractFirstCid(String json) {
        // Simple extraction: find "cid":NUMBER
